...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: `Contactez Salaicha Parfumeur à ${site.address}. WhatsApp Business, e-mail et horaires d'ouverture.` },
      { property: "og:title", content: "Contact — Salaicha Parfumeur" },
      { property: "og:description", content: "Tous les moyens de nous joindre : WhatsApp, e-mail, adresse et horaires." },
    ],
  }),
  component: Contact,
});
  ...
  return (
    <section className="container-x py-16">
      ...
      <p className="mt-4 max-w-2xl text-foreground/70">
        Notre équipe est à votre écoute pour toute question sur nos parfums, votre commande ou la livraison.
        La voie la plus rapide reste WhatsApp Business.
      </p>

      <div className="mt-10 grid gap-10 lg:grid-cols-2">
        <div className="space-y-4">
          {[
            { icon: MessageCircle, label: "WhatsApp Business", value: site.phoneDisplay, href: whatsappLink("Bonjour, je vous contacte depuis votre site.") },
            { icon: Phone, label: "Téléphone", value: site.phoneDisplay, href: `tel:+${site.whatsapp}` },
            ...
            <div key={label} className="flex items-start gap-4 rounded-2xl border border-border bg-card p-5">
              ...
              <div>
                ...
                {href ? (
                  <a href={href} target={href.startsWith("http") ? "_blank" : undefined} rel="noopener noreferrer" className="font-medium hover:text-primary">
                    {value}
                  </a>
                ) : (
