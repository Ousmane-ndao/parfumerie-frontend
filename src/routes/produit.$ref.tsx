...
function ProductPage() {
  ...
  const related = products
    .filter((x) => x.type === p.type && x.ref !== p.ref && (p.type !== "Parfum" || x.family === p.family))
    .slice(0, 3);

  return (
    <>
      <div className="container-x pt-8">
        <Link to="/catalogue" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-primary">
      ...
      <section className="container-x py-10 grid gap-12 md:grid-cols-2">
        <div className="relative">
          ...
          <img src={p.image} alt={p.name} width={1024} height={1024} className="relative rounded-2xl shadow-elegant w-full aspect-square object-cover" />
        </div>
        <div>
          <p className="text-xs uppercase tracking-widest text-rose-deep">{productDisplayBadge(p)} · Réf. {p.ref}</p>
          ...
          <dl className="mt-7 grid grid-cols-2 gap-y-3 text-sm">
            <dt className="text-muted-foreground">{parfum ? "Contenance" : "Capacité"}</dt><dd>{p.contenance}</dd>
            <dt className="text-muted-foreground">{parfum ? "Concentration" : "Type"}</dt><dd>{p.concentration}</dd>
          ...
          <div className="mt-8 rounded-2xl border border-border bg-card p-5">
            <h3 className="font-display text-xl text-primary">{parfum ? "Pyramide olfactive" : "Caractéristiques"}</h3>
            <div className="mt-3 space-y-2 text-sm">
              <p><span className="font-medium text-rose-deep">{parfum ? "Notes de tête :" : "Points forts :"}</span> {p.notes.top.join(", ")}</p>
              <p><span className="font-medium text-rose-deep">{parfum ? "Notes de cœur :" : "Fonctionnalités :"}</span> {p.notes.heart.join(", ")}</p>
              <p><span className="font-medium text-rose-deep">{parfum ? "Notes de fond :" : "Idéal pour :"}</span> {p.notes.base.join(", ")}</p>
            </div>
          </div>
          ...
          <p className="mt-3 text-xs italic text-muted-foreground">
            Aucun paiement en ligne. Modalités de paiement et de livraison convenues directement via WhatsApp Business.
          </p>
        </div>
      </section>
      ...
      {related.length > 0 && (
        <section className="container-x py-14">
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {related.map((r) => (
              <Link key={r.ref} to="/produit/$ref" params={{ ref: r.ref }} className="group overflow-hidden rounded-2xl border border-border bg-card transition-shadow hover:shadow-elegant">
                <div className="overflow-hidden aspect-square bg-muted">
                  <img src={r.image} alt={r.name} loading="lazy" width={1024} height={1024} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" />
                </div>
                <div className="p-5">
